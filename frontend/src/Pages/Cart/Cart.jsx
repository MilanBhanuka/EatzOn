               ...
               <div className="mt-10 p-6">
                <div className="cart-items">
                        ...
                        {food_list.map((item,index) => {
                            if(cartItems[item._id]>0){
                                return(
                                    <div key={item._id} className="grid grid-cols-8 bg-red-100 justify-center items-center rounded-lg ">
                                        <img className="col-span-2 h-24 rounded-l-lg" src={item.image} alt="food" />
                                        <p className="col-span-2 justify-center flex ">{item.name}</p>
                                        <p className="col-span-1 justify-center flex">${item.price}</p>
                                        <p className="col-span-1 justify-center flex">{cartItems[item._id]}</p>
                                        <p className="col-span-1 justify-center flex">${item.price*cartItems[item._id]}</p>
                                        <button className="col-span-1 justify-center flex " onClick={()=>removeFromCart(item._id)}><img src={assets.cross_icon} alt="cross" className="w-3 cursor-pointer"/></button>
                                    </div>
                                ) 
                            }
                        })}
                ...
                <div className="mt-4 grid grid-cols-1 sm:grid-cols-2  justify-between gap-5">
                        <div className="flex flex-col shadow-lg p-3 rounded-lg">
                                ...
                                <button className="bg-red-900 hover:bg-red-700 py-1 mt-4 rounded-lg lg:w-1/3">Proceed To Checkout</button>
                        </div>
                        <div className="flex flex-col shadow-lg p-3 rounded-lg">
                               <div className="flex flex-col">
                                <p className="mb-3">If you have a coupon code, please apply it below.</p>
                                <div className="flex">
                                        <input className="border-2 border-red-900 p-1 rounded-l-lg outline-none " type="text" placeholder="Promocode"/>
                                        <button className="bg-red-900 rounded-r-lg px-4 hover:bg-red-700  ">Apply</button>
                                </div>
                                </div> 
                        </div>
                </div>
